...
public abstract class BaseObject<T extends Annotation> implements Serializable {
	        ...
	        for (AccessibleObject prop : getBusinessKey()) {
	            try {
	                ...
	                if (prop instanceof Field) {
	                	...
	                	name = name.startsWith("is") ? name.substring(2) : name.startsWith("get") ? name.substring(3) : name;
	...
	/**
	 * When Hibernate proxies objects, it masks the type of the actual entity object. This wrapper
	...
	protected boolean hasSameObjectSignatureAs(BaseObject<T> compareTo) {
		if (!getBusinessKey().isEmpty()) {
			...
			for (AccessibleObject prop : getBusinessKey()) {
				try {
					if (prop instanceof Field) {
						builder.append(((Field) prop).get(this), ((Field) prop).get(compareTo));
					} else {
						builder.append(((Method) prop).invoke(this, (Object[]) null), ((Method) prop).invoke(compareTo, (Object[]) null));
					}
	            } catch (Exception e) {
	            	throw new UnsupportedOperationException(e);
	            }
	...
	private List<AccessibleObject> getBusinessKey() {
